...
OMX_ERRORTYPE MPEG2VIDDEC_AllocateResources(MYDATATYPE* pAppData)
{
    ...
    pAppData->pMpeg2 = (OMX_VIDEO_PARAM_MPEG2TYPE*)malloc(sizeof(OMX_VIDEO_PARAM_MPEG2TYPE));
    if (!pAppData->pMpeg2) {
        ERR_PRINT("%s :OMX_ErrorInsufficientResources: %d\n", __FUNCTION__, __LINE__);
        eError = OMX_ErrorInsufficientResources;
        goto EXIT;
    }
    memset(pAppData->pMpeg2, VAL_ZERO, sizeof(OMX_VIDEO_PARAM_MPEG2TYPE));
...
OMX_ERRORTYPE MPEG2VIDDEC_SetParamPortDefinition(MYDATATYPE* pAppData)
{
    ...
    pAppData->pInPortDef->format.video.eCompressionFormat       = PORTCOMPRESSIONFORMATMPEG2;
...
void MPEG2VIDDEC_FreeResources(MYDATATYPE* pAppData)
{
    ...
    if (pAppData->pMpeg2 != NULL){
        free(pAppData->pMpeg2);
        pAppData->pMpeg2 = NULL;
    }
    if (pAppData->pStartTime != NULL){
